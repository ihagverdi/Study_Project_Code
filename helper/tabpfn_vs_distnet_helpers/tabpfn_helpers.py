...
def calculate_nllh_tabpfn(model, X_valid, y_valid, validation_batch_size, target_scale, args=None):
    ...
    assert X_valid.ndim == 2 and y_valid.ndim == 2, "X_valid and y_valid must be 2D arrays"
    assert X_valid.shape[0] == y_valid.shape[0], "X_valid and y_valid must have the same number of instances"

    nllh = 0.0
    n_validation_instances = X_valid.shape[0]  # total number of validation instances
    ...
    for start in range(0, n_validation_instances, validation_batch_size):
        X_batch = X_valid[start: start + validation_batch_size]
        ...
        with torch.no_grad():
            ...
            for instance_idx, obss in enumerate(y_batch):
                ...
                # criterion.pdf returns exp(-log(p))
                nlog_pdf = torch.log(criterion.pdf(logits_rep, obs_t))  # -log(p) of shape (n,)
                
                # Cap infinite values to a maximum penalty (50 nats ≈ prob of 2e-22)
                max_nll = 50.0
                nlog_pdf = torch.clamp(nlog_pdf, max=max_nll)
                ...
                if target_scale == "log":
                    ...
                    raise ValueError("target_scale must be 'max' or 'log' or 'z-score'")
